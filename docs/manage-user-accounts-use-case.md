...
- **Add User Account:** Admin creates Student, SBO Officer, and Department Head accounts through `POST /users`. SAO creates organization Admin accounts through the separate `/system/admins` API. Neither role can create another Super Admin through the application.
