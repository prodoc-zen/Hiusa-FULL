...
test('deterministic AI service applies forecast, budget, and delegation rules', async ({ request }) => {
  ...
  const delegation = await request.post(`${aiUrl}/api/v1/task-delegation`, {
    ...
    data: {
      ...
      max_active_tasks: 5,
      officers: [
        { officer_id: 1, name: 'Busy Officer', role: 'SBO_OFFICER', account_status: 'active', active_tasks: 3, completed_tasks: 1, overdue_tasks: 1 },
        { officer_id: 2, name: 'Available Officer', role: 'SBO_OFFICER', account_status: 'active', active_tasks: 0, completed_tasks: 3, overdue_tasks: 0 },
