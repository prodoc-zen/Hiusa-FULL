...
export default function ScannerStatus({ reader }) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-lg border p-3 ${tone}`} role="status">
      ...
      {!reader.connected && <button type="button" onClick={reader.retry} aria-label="Retry fingerprint reader" className="grid h-11 w-11 shrink-0 place-items-center rounded-lg border border-current/20 bg-white/70"><RefreshCw size={15} /></button>}
    </div>
  );
}
