  ...
  return <div className="space-y-5">
    <section className="rounded-lg border border-[#DDE7EF] bg-white p-5 shadow-sm">
      ...
      <form onSubmit={submit} className="mt-6 border-t border-[#DDE7EF] pt-5">
        ...
        <button disabled={busy || !form.name.trim()} className="mt-5 inline-flex h-11 items-center gap-2 rounded-lg bg-[#0878B7] px-4 text-sm font-bold text-white hover:bg-[#0F2F62] disabled:opacity-50"><Plus size={16} />{busy ? 'Saving...' : 'Add Program'}</button>
      </form>
    ...
    {deleteTarget && <AccessibleOverlay label="Delete academic program" onClose={() => !busy && setDeleteTarget(null)} className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B1831]/50 p-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-lg bg-white p-6 shadow-2xl"><h3 className="text-lg font-black text-[#0F172A]">Delete program?</h3><p className="mt-2 text-sm text-slate-600">This permanently removes <strong>{deleteTarget.name}</strong> and all its sections. Programs assigned to users cannot be deleted.</p><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setDeleteTarget(null)} disabled={busy} className="h-11 rounded-lg border border-[#DDE7EF] px-4 text-sm font-bold text-slate-600">Cancel</button><button type="button" onClick={confirmDelete} disabled={busy} className="inline-flex h-11 items-center gap-2 rounded-lg bg-red-600 px-4 text-sm font-bold text-white disabled:opacity-50"><Trash2 size={15} />{busy ? 'Deleting...' : 'Delete Program'}</button></div></div></AccessibleOverlay>}
  </div>;
}
