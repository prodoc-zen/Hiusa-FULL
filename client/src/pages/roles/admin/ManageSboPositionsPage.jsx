...
export default function ManageSboPositionsPage() {
  ...
  return <div className="space-y-5">
    ...
    <section className="flex flex-col gap-4 rounded-lg border border-[#DDE7EF] bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      <div><p className="text-[10px] font-bold uppercase tracking-widest text-[#0878B7]">Organization access structure</p><h2 className="mt-1 text-2xl font-black text-[#0F172A]">Manage Positions</h2><p className="mt-1 max-w-2xl text-sm text-slate-500">Create and maintain positions separately for Admin and SBO Officer accounts. Students and Department Heads cannot be assigned these positions.</p></div>
      <button type="button" onClick={openCreate} className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-lg bg-[#0878B7] px-4 text-sm font-bold text-white hover:bg-[#0F2F62]"><Plus size={16} />Add Position</button>
    </section>
    ...
    <Modal open={Boolean(formMode)} title={formMode === 'edit' ? 'Edit Position' : 'Add Position'} description="Positions determine which titles can be assigned in Manage Users." onClose={closeForm} closeOnBackdrop={!busy} closeOnEscape={!busy} footer={<><button type="button" onClick={closeForm} disabled={busy} className="h-10 rounded-lg border border-[#DDE7EF] px-4 text-sm font-bold text-slate-600">Cancel</button><button type="submit" form="position-form" disabled={busy || !form.title.trim()} className="h-10 rounded-lg bg-[#0878B7] px-4 text-sm font-bold text-white disabled:opacity-50">{busy ? 'Saving...' : 'Save Position'}</button></>}>
