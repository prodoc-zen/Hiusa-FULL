...
import { CalendarClock, Download, Eye, FileSpreadsheet, FileText, Search } from 'lucide-react';
...
function exportRows(rows, filename, title, print = false) {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${escape(title)}</title><style>body{font-family:Arial;padding:24px;color:#0f172a}table{border-collapse:collapse;width:100%;font-size:12px}th,td{border:1px solid #cbd5e1;padding:8px;text-align:left}th{background:#eef6fb}</style></head><body><h1>${escape(title)}</h1><table><thead><tr>${headers.map((header) => `<th>${escape(header)}</th>`).join('')}</tr></thead><tbody>${exportableRows.map((row) => `<tr>${headers.map((header) => `<td>${escape(row[header])}</td>`).join('')}</tr>`).join('')}</tbody></table></body></html>`;
  if (print) {
...
export default function SuperAdminFinancialReportsPage() {
  ...
  return <div className="space-y-5">
    <section className="rounded-xl border border-[#DDE7EF] bg-white p-5"><p className="text-[10px] font-bold uppercase tracking-widest text-[#0B8ED0]">Student Affairs Office</p><h1 className="mt-1 text-2xl font-black text-[#0F172A]">Financial Reports & Transactions</h1><p className="mt-1 text-sm text-slate-500">University-wide, read-only financial oversight with organization filters.</p></section>
    <section className="grid gap-4 xl:grid-cols-[1fr_360px]">
      <div className="rounded-xl border border-[#DDE7EF] bg-white p-5"><div className="grid gap-3 sm:grid-cols-3"><div><p className="text-xs font-bold text-slate-500">Inflows</p><p className="mt-1 text-xl font-black text-emerald-700">{money(summary.total_income)}</p></div><div><p className="text-xs font-bold text-slate-500">Outflows</p><p className="mt-1 text-xl font-black text-red-600">{money(summary.total_expense)}</p></div><div><p className="text-xs font-bold text-slate-500">Net</p><p className="mt-1 text-xl font-black text-[#0F172A]">{money(summary.net_balance)}</p></div></div></div>
      <form onSubmit={saveDeadline} className="rounded-xl border border-[#DDE7EF] bg-white p-5"><div className="flex items-center gap-2"><CalendarClock size={18} className="text-[#0B8ED0]"/><h2 className="font-bold text-[#0F172A]">Submission deadline</h2></div><p className="mt-1 text-xs text-slate-500">Current: {date(deadline?.deadline_at)}</p><label className="mt-3 block text-xs font-bold text-slate-600">New deadline<input required type="datetime-local" value={deadlineForm.deadline_at} onChange={(event) => setDeadlineForm({ ...deadlineForm, deadline_at: event.target.value })} className="mt-1 h-11 w-full rounded-lg border border-[#DDE7EF] px-3" /></label><label className="mt-3 block text-xs font-bold text-slate-600">Instructions<textarea value={deadlineForm.instructions} onChange={(event) => setDeadlineForm({ ...deadlineForm, instructions: event.target.value })} rows={2} className="mt-1 w-full rounded-lg border border-[#DDE7EF] p-3" /></label><button disabled={busy} className="mt-3 h-11 w-full rounded-lg bg-[#0B8ED0] px-4 text-sm font-bold text-white disabled:opacity-50">{busy ? 'Saving…' : deadline ? 'Update deadline' : 'Set deadline'}</button></form>
    </section>
    <section className="rounded-xl border border-[#DDE7EF] bg-white p-5"><div className="grid gap-3 md:grid-cols-6"><select aria-label="Organization" value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className="h-11 rounded-lg border border-[#DDE7EF] px-3 text-sm"><option value="">All organizations</option>{organizations.map((org) => <option key={org.id} value={org.id}>{org.acronym} - {org.name}</option>)}</select><label className="relative md:col-span-2"><Search size={16} className="absolute left-3 top-3.5 text-slate-400"/><input aria-label="Search transactions" placeholder="Search transactions" value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} className="h-11 w-full rounded-lg border border-[#DDE7EF] pl-9 pr-3 text-sm" /></label><input aria-label="Event filter" placeholder="Filter by event" value={filters.event_search} onChange={(event) => setFilters({ ...filters, event_search: event.target.value })} className="h-11 rounded-lg border border-[#DDE7EF] px-3 text-sm"/><select aria-label="Transaction type" value={filters.type} onChange={(event) => setFilters({ ...filters, type: event.target.value })} className="h-11 rounded-lg border border-[#DDE7EF] px-3 text-sm"><option value="">All types</option><option value="income">Income</option><option value="expense">Expense</option></select><button type="button" onClick={() => load(1)} className="h-11 rounded-lg bg-[#0B1831] px-4 text-sm font-bold text-white">Apply filters</button><input aria-label="From date" type="date" value={filters.from} onChange={(event) => setFilters({ ...filters, from: event.target.value })} className="h-11 rounded-lg border border-[#DDE7EF] px-3 text-sm"/><input aria-label="To date" type="date" value={filters.to} onChange={(event) => setFilters({ ...filters, to: event.target.value })} className="h-11 rounded-lg border border-[#DDE7EF] px-3 text-sm"/></div></section>
    <section className="overflow-hidden rounded-xl border border-[#DDE7EF] bg-white"><div className="border-b border-[#DDE7EF] p-5"><h2 className="font-bold text-[#0F172A]">Transaction history</h2></div>{loading ? <p className="p-8 text-center text-sm text-slate-400">Loading…</p> : <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-[#F8FBFD] text-xs uppercase text-slate-500"><tr>{['Date','Organization','Description','Event','Type','Amount'].map((item) => <th key={item} className="px-4 py-3">{item}</th>)}</tr></thead><tbody className="divide-y divide-[#E5EDF3]">{transactions.map((row) => <tr key={row.id}><td className="px-4 py-3">{date(row.transaction_date)}</td><td className="px-4 py-3 font-semibold">{row.organization?.acronym}</td><td className="px-4 py-3">{row.description}</td><td className="px-4 py-3">{row.event?.title || '—'}</td><td className="px-4 py-3 capitalize">{row.type}</td><td className="px-4 py-3 font-bold">{money(row.amount)}</td></tr>)}</tbody></table></div>}<PaginationControls currentPage={meta.current_page || 1} totalItems={meta.total || 0} pageSize={meta.per_page || 10} onPageChange={(page) => load(page)} label="transactions" /></section>
    <section className="rounded-xl border border-[#DDE7EF] bg-white"><div className="border-b border-[#DDE7EF] p-5"><h2 className="font-bold text-[#0F172A]">Submitted report register</h2></div><div className="divide-y divide-[#E5EDF3]">{reports.map((report) => <article key={report.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="font-bold text-[#0F172A]">{report.title}</p><p className="mt-1 text-xs text-slate-500">{report.organization?.acronym} · {statusLabel(report.submission_status)} · {date(report.generated_at)}</p></div><button type="button" onClick={() => openReport(report)} className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-[#DDE7EF] px-3 text-xs font-bold"><Eye size={14}/> View details</button></article>)}{!reports.length && <p className="p-8 text-center text-sm text-slate-400">No reports found.</p>}</div></section>
    <Modal open={Boolean(detail)} title={detail?.report?.title || 'Financial report'} description={detail?.report ? `${detail.report.organization?.name || ''} · ${statusLabel(detail.report.submission_status)}` : ''} onClose={() => setDetail(null)} maxWidth="max-w-4xl" footer={<><button type="button" onClick={() => exportReport('excel')} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[#0B8ED0] px-4 text-sm font-bold text-white"><FileSpreadsheet size={15}/>Excel</button><button type="button" onClick={() => exportReport('pdf')} className="inline-flex h-10 items-center gap-2 rounded-lg border border-[#DDE7EF] px-4 text-sm font-bold"><FileText size={15}/>PDF</button></>}>
      {detail && <div className="space-y-4"><p className="text-sm leading-6 text-slate-600">{detail.report.summary_text}</p><div className="grid gap-3 sm:grid-cols-2"><div><h3 className="text-sm font-bold">Required signatories</h3>{Object.entries(detail.report.signatories || {}).map(([role, name]) => <p key={role} className="mt-1 text-sm capitalize text-slate-600">{role.replaceAll('_',' ')}: <strong>{name}</strong></p>)}</div><div><h3 className="text-sm font-bold">Supporting documents</h3>{(detail.report.supporting_documents || []).map((doc) => <a key={doc.path} href={resolveAssetUrl(doc.url)} target="_blank" rel="noreferrer" className="mt-1 flex items-center gap-2 text-sm font-bold text-[#0B8ED0]"><Download size={14}/>{doc.name}</a>)}{!(detail.report.supporting_documents || []).length && <p className="mt-1 text-sm text-slate-400">No files attached.</p>}</div></div></div>}
    </Modal>
