...
import { getUsers } from '../../services/electionService';
...
export default function ManageVotersPage() {
  ...
  const voterIds = new Set((election.votes || []).map((vote) => vote.voter_id));
  const voters = users.map((user) => ({
    ...user,
    has_voted: voterIds.has(user.id),
  }));
        ...
        {[
          {
            ...
            value: voters.length,
            icon: Users,
            color: { bg: 'bg-[#E6F6FD]', icon: 'text-[#0B8ED0]', border: 'border-[#0B8ED0]/20' },
          },
          {
            label: 'Voted',
            value: votedCount,
          ...
          <div key={stat.label} className={`flex items-start gap-4 rounded-xl border bg-white p-5 shadow-sm ${stat.color.border}`}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-2xl font-black text-[#0F172A]">{stat.value}</p>
        ...
        <div className="flex flex-wrap gap-3 p-5 pb-0">
          <div className="relative min-w-[200px] flex-1">
            ...
            <input
              ...
              className="h-10 w-full rounded-lg border border-[#DDE7EF] bg-[#F8FBFD] pl-9 pr-4 text-sm outline-none transition placeholder:text-[#94A3B8] focus:border-[#0B8ED0] focus:ring-4 focus:ring-[#16C7F3]/15"
            />
          </div>
          <div className="flex gap-1.5">
            {['All', 'Voted', 'Not Yet'].map((status) => (
              <button
                key={status}
                ...
                className={`rounded-lg px-3 py-2 text-xs font-bold transition ${
                  filterStatus === status
                    ...
                    : 'border border-[#DDE7EF] bg-[#F8FBFD] text-slate-600 hover:bg-[#EEF6FB]'
                }`}
              >
                {status}
              </button>
