...
export default function AnnouncementsFeedPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-5">
      <section className="rounded-lg border border-[#DDE7EF] bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="grid h-11 w-11 place-items-center rounded-lg bg-[#EEF6FB] text-[#0F2F62]"><Megaphone size={21} /></div>
            <p className="mt-4 text-[11px] font-bold uppercase tracking-widest text-[#0878B7]">HIUSA bulletin</p>
            <h1 className="mt-1 text-2xl font-black text-[#0F172A] sm:text-3xl">Announcements Feed</h1>
            <p className="mt-2 text-sm leading-6 text-[#64748B]">Updates, schedules, and important notices from your organization in one readable feed.</p>
          </div>
          <div className="rounded-lg border border-[#DDE7EF] bg-[#F8FBFD] px-4 py-3 text-left lg:text-right">
            <p className="text-2xl font-black text-[#0878B7]">{meta.total}</p>
      ...
      {loading && <div className="grid gap-4 md:grid-cols-2" role="status" aria-label="Loading announcements">{[1, 2, 3].map((item) => <div key={item} className="h-72 animate-pulse rounded-lg border border-[#DDE7EF] bg-slate-100" />)}<span className="sr-only">Loading announcements...</span></div>}
      {error && <div className="rounded-lg border border-red-200 bg-red-50 p-5 text-center"><p className="text-sm font-semibold text-red-700">{error}</p><button type="button" onClick={load} className="mt-2 text-sm font-bold text-red-600 underline">Try again</button></div>}
      {!loading && !error && filtered.length === 0 && <div className="rounded-lg border border-dashed border-[#DDE7EF] bg-white p-10 text-center"><Megaphone size={36} className="mx-auto text-[#94A3B8]" /><h2 className="mt-3 text-base font-bold text-[#0F172A]">No announcements to show</h2><p className="mt-1 text-sm text-[#64748B]">Try another search or category.</p></div>}

      {!loading && !error && featured && (
        <article className="grid overflow-hidden rounded-lg border border-[#DDE7EF] bg-white shadow-sm lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
          ...
          <div className="flex flex-col justify-between p-5 sm:p-7">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest text-[#0878B7]">Audience</p>
              <p className="mt-2 text-xl font-black text-[#0F172A]">{ROLE_LABEL[featured.target_role] ?? featured.target_role}</p>
              <div className="mt-5 flex items-start gap-2 rounded-lg border border-[#DDE7EF] bg-[#F8FBFD] p-3 text-xs leading-5 text-[#64748B]"><CalendarDays size={15} className="mt-0.5 shrink-0 text-[#0878B7]" />Published {formatDate(featured.created_at)}</div>
            </div>
