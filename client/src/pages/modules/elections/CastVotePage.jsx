...
export default function CastVotePage({ election: electionOverride = null, refreshElection: refreshElectionOverride = null, onReturnToDashboard = null, onDirtyChange = null }) {
  ...
  if (!hasVoted && votingPeriodRestriction) {
    return <div className="mx-auto max-w-2xl rounded-lg border border-amber-200 bg-amber-50 p-8 text-center"><CalendarDays size={34} className="mx-auto text-amber-600" /><h2 className="mt-4 text-xl font-black text-amber-900">{votingPeriodRestriction.title}</h2><p className="mt-2 text-sm font-medium text-amber-800">{votingPeriodRestriction.message}</p></div>;
  }

  if ((hasVoted && phase !== 'submitted') || alreadyVotedDetected) {
    return (
      <Modal open title="Vote Already Recorded" description="Each voter can submit only one ballot for this election." onClose={returnToPreviousPage} closeOnBackdrop closeOnEscape maxWidth="max-w-md" footer={<button type="button" data-autofocus onClick={returnToPreviousPage} className="h-11 rounded-lg bg-[#0878B7] px-5 text-sm font-bold text-white hover:bg-[#0F2F62]">Go back</button>}>
  ...
  if (phase === 'submitted') {
    ...
    return (
      <div className="mx-auto max-w-3xl overflow-hidden rounded-lg border border-[#DDE7EF] bg-white shadow-sm">
  ...
  if (phase === 'preview') {
    return (
      <div className="space-y-5">
        <section className="grid overflow-hidden rounded-lg border border-[#DDE7EF] bg-white shadow-sm lg:grid-cols-[minmax(320px,.8fr)_minmax(0,1.2fr)]">
          ...
          <div className="p-5 sm:p-7 lg:p-8">
            <p className="text-[11px] font-bold uppercase tracking-widest text-[#0878B7]">Official ballot</p>
            ...
            <button type="button" onClick={() => { setPositionIndex(0); setPhase('voting'); }} className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#0878B7] px-5 text-sm font-bold text-white hover:bg-[#0F2F62] sm:w-auto">Start my ballot <ArrowRight size={16} /></button>
          </div>
        </section>

        <section className="overflow-hidden rounded-lg border border-[#DDE7EF] bg-white" aria-label="How secure voting works">
          <div className="border-b border-[#DDE7EF] px-5 py-4"><h2 className="text-base font-black text-[#0F172A]">How secure voting works</h2><p className="mt-1 text-xs text-[#64748B]">Your ballot stays editable until the final confirmation.</p></div>
          <ol className="grid md:grid-cols-3 md:divide-x md:divide-[#DDE7EF]">
            {[{ icon: UsersRound, number: '01', title: 'Review candidates', text: 'Read each candidate’s party and platform.' }, { icon: Check, number: '02', title: 'Complete the ballot', text: 'Choose one candidate for every position.' }, { icon: ShieldCheck, number: '03', title: 'Confirm once', text: 'Review all selections before secure submission.' }].map((step) => <li key={step.number} className="flex gap-3 border-b border-[#DDE7EF] p-4 last:border-b-0 md:border-b-0"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#E6F6FD] text-[#0F2F62]"><step.icon size={17} /></span><div><p className="text-[10px] font-black text-[#0878B7]">STEP {step.number}</p><h3 className="mt-0.5 text-sm font-black text-[#0F172A]">{step.title}</h3><p className="mt-1 text-xs leading-5 text-[#64748B]">{step.text}</p></div></li>)}
          </ol>
        </section>
      </div>
    );
  ...
  if (phase === 'review') {
    ...
    return (
      ...
      <div className="mx-auto max-w-4xl space-y-4">
        <section className="rounded-lg border border-[#DDE7EF] bg-white p-5 shadow-sm sm:p-7"><p className="text-[11px] font-bold uppercase tracking-widest text-[#0878B7]">Final check</p><h2 className="mt-1 text-2xl font-black text-[#0F172A]">Review your ballot</h2><p className="mt-2 text-sm text-[#64748B]">Submitted ballots cannot be edited.</p>
