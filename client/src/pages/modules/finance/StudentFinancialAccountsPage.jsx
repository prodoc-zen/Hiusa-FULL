...
export default function StudentFinancialAccountsPage() {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-lg border border-[#0F2F62] bg-[#0F2F62] p-5 text-white sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-widest text-[#16C7F3]">Student accountability</p><h1 className="mt-1 text-2xl font-black">Student Financial Accounts</h1><p className="mt-1 max-w-3xl text-sm leading-6 text-slate-200">Review outstanding charges, overdue balances, merchandise reservations, payment history, and clearance status.</p></div><button type="button" onClick={() => { setCharge({ student_id: '', description: '', amount_due: '', due_date: '' }); setFormError(''); }} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-white px-4 text-sm font-bold text-[#0F2F62] hover:bg-[#EEF6FB] sm:w-auto"><FilePlus2 size={16} /> Add Student Charge</button></div>
    </section>

    <section className="grid gap-px overflow-hidden rounded-lg border border-[#DDE7EF] bg-[#DDE7EF] sm:grid-cols-2 xl:grid-cols-4">
      {[
        { label: 'Total Outstanding', value: money(summary.total_outstanding), detail: `${summary.students_owing || 0} students with balances`, icon: WalletCards, tone: 'bg-[#E6F6FD] text-[#0F2F62]' },
        { label: 'Students Owing', value: summary.students_owing || 0, detail: `${summary.students_cleared || 0} financially cleared`, icon: AlertTriangle, tone: 'bg-amber-50 text-amber-700' },
        { label: 'Overdue Accounts', value: summary.students_overdue || 0, detail: 'Past at least one due date', icon: AlertTriangle, tone: 'bg-red-50 text-red-700' },
        { label: 'Cleared Students', value: summary.students_cleared || 0, detail: `Out of ${summary.total_students || 0} students`, icon: BadgeCheck, tone: 'bg-emerald-50 text-emerald-700' },
      ].map((item) => <dl key={item.label} className="flex items-start justify-between gap-3 bg-white p-4 sm:p-5"><div><dt className="text-xs font-semibold text-slate-500">{item.label}</dt><dd className="mt-2 text-2xl font-black tabular-nums text-[#0F172A]">{item.value}</dd><p className="mt-1 text-xs font-medium text-slate-500">{item.detail}</p></div><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-lg ${item.tone}`}><item.icon size={19} /></span></dl>)}
    </section>
    ...
    <section className="rounded-lg border border-[#DDE7EF] bg-white p-4 shadow-sm">
      <div className="grid gap-3 lg:grid-cols-[minmax(240px,1.4fr)_repeat(4,minmax(140px,1fr))]">
        <label className="relative"><span className="sr-only">Search accounts</span><Search size={16} className="absolute left-3 top-3.5 text-slate-500" /><input value={filters.search} onChange={(event) => updateFilter('search', event.target.value)} placeholder="Name, school ID, email, course..." className="h-11 w-full rounded-lg border border-[#DDE7EF] pl-9 pr-3 text-sm outline-none focus:border-[#0B8ED0] focus:ring-4 focus:ring-[#16C7F3]/15" /></label>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2"><button type="button" onClick={clearFilters} className="text-xs font-bold text-[#0878B7] hover:underline">Clear all filters</button><span className="rounded-lg border border-[#DDE7EF] bg-[#F8FBFD] px-3 py-2 text-xs font-semibold text-slate-500">10 rows per page</span></div>
    </section>
